...
pub fn ThreadHashMap(comptime ValueType: type, comptime count: anytype, comptime remove_trailing_slashes: bool) type {
    const max_index = count - 1;
    return struct {
        const Self = @This();
        const Overflow = OverflowList(ValueType, count / 4);

        index: IndexMapManaged,
        overflow_list: Overflow = Overflow{},
        ...
        pub fn isOverflowing(self: *Self) bool {
            return self.backing_buf_used >= @as(u16, count);
        }
        ...
        pub fn getOrPut(self: *Self, denormalized_key: []const u8) !Result {
            ...
            var index = try self.index.getOrPut(_key);
        ...
        pub fn markNotFound(self: *Self, result: Result) void {
            self.index.put(result.hash, NotFound) catch unreachable;
        }
        ...
        pub fn put(self: *Self, result: *Result, value: ValueType) !*ValueType {
            ...
            try self.index.put(result.hash, result.index);
